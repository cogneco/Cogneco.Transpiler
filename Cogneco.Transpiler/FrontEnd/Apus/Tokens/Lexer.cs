...
{
	public class Lexer : Generic.IEnumerable<Token>, IDisposable
	{
		...
		Lexer(IO.ICharacterReader reader)
		{
			this.reader = reader;
		}
		Text.Mark Mark()
		...
		public System.Collections.Generic.IEnumerator<Token> GetEnumerator()
		{
			...
			while (!empty)
			{
				...
				else if (this.StartsNumber(this.reader.Last))
				{
					...
					new Exception.LexicalError("a valid token", "invalid character (" + char.GetNumericValue(this.reader.Last).ToString("x") + ")", mark).Throw();
			}
		}
		...
		{
			return reader.NotNull() ? new Lexer(reader) : null;
		}
	}
}
